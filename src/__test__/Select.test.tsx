...
test("should correctly set default option", async () => {
  ...
  const selectInput = await screen.findByRole("option", {
    name: "Choose a dog breed",
  });
...
test("should change selected option", async () => {
  ...
  userEvent.selectOptions(
    ...
    await screen.findByRole("option", { name: "bulldog" })
  );
...
test("select name and display data", async () => {
  ...
  userEvent.selectOptions(
    screen.getByRole("combobox"),
    await screen.findByRole("option", { name: "african" })
  );
